	...
	constructor(settings: IRoseSettings = {}) {
		...
		this.loop = {
			start: 0,
				...
				Rose.getFinalAngleFromK(
					this.getProp<IRoseProps>('n', propArguments) as number,
					this.getProp<IRoseProps>('d', propArguments) as number
				),
			inc: (propArguments: ISceneChildPropArguments) => {
				const n = this.getProp<IRoseProps>('n', propArguments) as number
				const d = this.getProp<IRoseProps>('d', propArguments) as number
