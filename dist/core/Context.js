...
export default {
    ...
    distance: (repetition, offsetFromCenter = [0, 0], scaleDistance = [1, 1]) => {
        if (repetition.type == RepetitionType.Matrix) {
            const matrixOffset = Vec2.create(offsetFromCenter);
    ...
    percW: (percentage, shape) => shape && shape.scene ? (shape.scene.width * percentage) / 100 : percentage,
    percH: (percentage, shape) => shape && shape.scene ? (shape.scene.height * percentage) / 100 : percentage,
};
