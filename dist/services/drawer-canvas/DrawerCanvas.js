...
var DrawerCanvas = /** @class */ (function (_super) {
	 ...
	 * @template K
	 * @param {K} name
	 * @param {IDrawOptions[K]} defaultValue
	 * @returns {IDrawOptions[K]}
	 * @memberof DrawerCanvas
	 */
	DrawerCanvas.prototype.getOption = function (name, defaultValue) {
		var _a
		return (_a = this.drawOptions[name]) !== null && _a !== void 0 ? _a : defaultValue
	}
	/**
	 *
	 *
	...
	DrawerCanvas.draw = function (scene, context, options, resolution) {
		...
		if (context) {
			...
			var ghostMultiplier_1 = bGhost_1 ? 1 - options.ghost_index / (options.ghosts + 0.5) : 1
			...
			if (simmetricLine > 0) {
				...
				for (var i = 0; i < simmetricLine; i++) {
					...
					context.lineWidth = 1
					context.moveTo(
						(a[0] - size / 2) * final_scale_1[0] + final_translate_1[0],
						(a[1] - size / 2) * final_scale_1[1] + final_translate_1[1]
					)
					context.lineTo(
						(b[0] - size / 2) * final_scale_1[0] + final_translate_1[0],
						(b[1] - size / 2) * final_scale_1[1] + final_translate_1[1]
					)
					context.stroke()
				}
			}
			...
			scene.stream(function (_a) {
				var lineWidth = _a.lineWidth,
					...
					frame_buffer_index = _a.frame_buffer_index
				if (shape.data && (shape.data.visible === false || (bGhost_1 && shape.data.disableGhost === true))) return
				if (shape.data && shape.data.composite) {
					context.globalCompositeOperation = shape.data.composite
				}
				context.beginPath()
				context.moveTo(
					(buffer[frame_buffer_index] - width_1 / 2) * final_scale_1[0] + final_translate_1[0],
					(buffer[frame_buffer_index + 1] - height_1 / 2) * final_scale_1[1] + final_translate_1[1]
				)
				for (var i = 2; i < frame_length; i += 2) {
					context.lineTo(
						(buffer[frame_buffer_index + i] - width_1 / 2) * final_scale_1[0] + final_translate_1[0],
						(buffer[frame_buffer_index + i + 1] - height_1 / 2) * final_scale_1[1] + final_translate_1[1]
					)
				}
