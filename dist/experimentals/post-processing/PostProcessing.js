import { drawScene, initBuffers, initShaderProgram, IProgramInfo, IUniforms, loadTexture, } from "./PostProcessingUtilities";
const noise = `
//
...
class PostProcessing {
    constructor(canvas) {
        this.vertex = baseVertexShader;
        this.fragment = baseFragmentShader;
        this.noise = noise;
                ...
                if (buffers) {
                    const render = () => {
                        if (this.gl_context && this.referer) {
                            ...
                            const texture = loadTexture(this.gl_context, this.referer.getImageData(0, 0, this.canvas.width, this.canvas.height));
                            this.uniforms.uTime.value = drawer.getTimeline().getTime();
                            if (texture)
                                drawScene(this.gl_context, programInfo, buffers, texture, this.uniforms);
                        }
                    };
                    requestAnimationFrame(render);
                }
